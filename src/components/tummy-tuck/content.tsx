...
import {
    tummyTuckIntro, tummyTuckWhenToConsider, tummyTuckIdealCandidateIntro, tummyTuckIdealCandidate,
    tummyTuckResults, tummyTuckAdvantages, tummyTuckNotRecommended,
    tummyTuckRisksIntro, tummyTuckRisks, tummyTuckRisksOutro, tummyTuckQuickFacts,
} from '@/constants/tummy-tuck'

export default function TummyTuckContent() {
    return (
            ...
            <Wrapper>
                ...
                <div className='grid lg:grid-cols-[1fr_280px] gap-12 lg:gap-16'>
                    <div className='flex flex-col gap-16'>
                        <div>
                            <RevealText as="h2" text="Tummy tuck surgery — abdominoplasty" className='font-serif text-2xl md:text-3xl text-plum-ink mb-4' />
                            {tummyTuckIntro.map((p, i) => (
                        ...
                        <div>
                            <RevealText as="h2" text="When to consider a tummy tuck" className='font-serif text-2xl md:text-3xl text-plum-ink mb-6' />
                            <StaggerContainer className='flex flex-col'>
                                {tummyTuckWhenToConsider.map((item) => (
                        ...
                        <div>
                            ...
                            <Reveal>
                                <p className='text-sm md:text-base text-warm-slate leading-relaxed max-w-xl mb-6'>{tummyTuckIdealCandidateIntro}</p>
                            </Reveal>
                            <StaggerContainer className='flex flex-col'>
                                {tummyTuckIdealCandidate.map((item) => (
                                    <StaggerItem key={item} className='flex items-start gap-3 py-2.5 border-b border-rose-100 last:border-0'>
                                        <span className='mt-0.5 shrink-0 size-5 rounded-full bg-rose-100 text-rose-700 flex items-center justify-center'>
                                            <Check size={12} strokeWidth={3} />
                                        </span>
                        ...
                        <div>
                            <RevealText as="h2" text="How long do results last?" className='font-serif text-2xl md:text-3xl text-plum-ink mb-4' />
                            {tummyTuckResults.map((p, i) => (
                                <Reveal key={i} delay={0.1 + i * 0.1}>
                                    <p className='text-sm md:text-base text-warm-slate leading-relaxed max-w-xl mb-4 last:mb-0'>{p}</p>
                                </Reveal>
                            ))}
                        ...
                        <div>
                            <RevealText as="h2" text="Who should not opt for abdominoplasty" className='font-serif text-2xl md:text-3xl text-plum-ink mb-6' />
                            <StaggerContainer className='flex flex-col'>
                                {tummyTuckNotRecommended.map((item) => (
                                    <StaggerItem key={item} className='flex items-start gap-3 py-2.5 border-b border-rose-100 last:border-0'>
                        ...
                        <div>
                            <RevealText as="h2" text="Risks associated with tummy tuck surgery" className='font-serif text-2xl md:text-3xl text-plum-ink mb-4' />
                            <Reveal>
                                <p className='text-sm md:text-base text-warm-slate leading-relaxed max-w-xl mb-6'>{tummyTuckRisksIntro}</p>
                            </Reveal>
                            <StaggerContainer className='grid sm:grid-cols-2 gap-4 mb-6'>
                                {tummyTuckRisks.map((item) => (
                                    <StaggerItem key={item}>
                            ...
                            <Reveal delay={0.1}>
                                <p className='text-sm text-warm-slate leading-relaxed max-w-xl'>{tummyTuckRisksOutro}</p>
                            </Reveal>
                        </div>
                    </div>
