...
import {
    pcodIntro, pcodCauses, pcodSymptoms, pcodComplications, pcodWhenToSeeDoctor,
    pcodDiagnosis, pcodTreatments, pcodLifestyle, pcodTestimonials, pcodFaqs,
} from '@/constants/pcod'

export default function PcodContent() {
    return (
            ...
            <Wrapper className='grid lg:grid-cols-[240px_1fr] gap-12 lg:gap-16'>
                ...
                <div className='flex flex-col gap-20'>
                    ...
                    <div className='grid md:grid-cols-2 gap-10 items-center'>
                        <div>
                            <RevealText as="h2" text="What is PCOD?" className='font-serif text-2xl md:text-3xl text-plum-ink' />
                        ...
                        <Reveal delay={0.15} className='relative w-full aspect-4/3 rounded-card overflow-hidden'>
                            <Image src="/images/gynaecology/pcod-overview-new.jpeg" alt="PCOD consultation" fill sizes="(min-width: 768px) 480px, 90vw" className='object-cover' />
                        </Reveal>
                    </div>

                    {/* Causes */}
                    <div>
                        <RevealText as="h2" text="What are the causes of PCOD?" className='font-serif text-2xl md:text-3xl text-plum-ink mb-8' />
                        <IconItemGrid items={pcodCauses} />
                    </div>

                    {/* Symptoms */}
                    <div>
                        <RevealText as="h2" text="Symptoms of PCOD" className='font-serif text-2xl md:text-3xl text-plum-ink mb-8' />
                        <StaggerContainer className='grid md:grid-cols-2 gap-x-10 gap-y-1'>
                            {pcodSymptoms.map((item) => (
                    ...
                    <div>
                        <RevealText as="h2" text="Complications in PCOD" className='font-serif text-2xl md:text-3xl text-plum-ink mb-8' />
                        <StaggerContainer className='grid sm:grid-cols-2 lg:grid-cols-3 gap-6'>
                            {pcodComplications.map((item) => (
                        ...
                        <Reveal delay={0.2}>
                            <div className='mt-8 p-6 rounded-card bg-rose-50 border border-rose-100'>
                                ...
                                <p className='text-sm text-warm-slate leading-relaxed'>{pcodWhenToSeeDoctor}</p>
                            </div>
                        </Reveal>
                    </div>
                    ...
                    {/* Diagnosis */}
                    <div>
                        <RevealText as="h2" text="Diagnosis of PCOD" className='font-serif text-2xl md:text-3xl text-plum-ink mb-8' />
                        <StaggerContainer className='flex flex-col'>
                            {pcodDiagnosis.map((step, i) => (
                    ...
                    {/* Treatment + lifestyle */}
                    <div>
                        <RevealText as="h2" text="Treatment of PCOD" className='font-serif text-2xl md:text-3xl text-plum-ink mb-8' />
                        <IconItemGrid items={pcodTreatments} />
                        <Reveal delay={0.2}>
                            <div className='mt-8 p-6 rounded-card bg-sage-50 border border-sage-100'>
                                <p className='font-serif text-lg text-plum-ink mb-2'>Lifestyle modifications to treat PCOD</p>
                                <p className='text-sm text-warm-slate leading-relaxed'>{pcodLifestyle}</p>
                            </div>
                        </Reveal>
                    </div>
                    ...
                    {/* FAQ */}
                    <div>
                        <RevealText as="h2" text="FAQs related to PCOD" className='font-serif text-2xl md:text-3xl text-plum-ink mb-4' />
                        <FaqAccordion items={pcodFaqs} />
                    </div>
                </div>
            </Wrapper>
        </Section>
