...
import {
    forcepsIntro, forcepsIndications, forcepsSteps, forcepsNotAdvisable, forcepsBenefits,
    forcepsMotherRisks, forcepsBabyRisks, forcepsAfterProcedure, forcepsRecoveryTips,
} from '@/constants/forceps-delivery'

export default function ForcepsDeliveryContent() {
    return (
        <Section className='bg-petal-white'>
            <Wrapper className='grid lg:grid-cols-[240px_1fr] gap-12 lg:gap-16'>
                ...
                <div className='flex flex-col gap-20'>
                    ...
                    <div>
                        <RevealText as="h2" text="What is forceps delivery?" className='font-serif text-2xl md:text-3xl text-plum-ink mb-4' />
                        {forcepsIntro.map((p, i) => (
                    ...
                    <div>
                        <RevealText as="h2" text="When is forceps delivery performed?" className='font-serif text-2xl md:text-3xl text-plum-ink mb-6' />
                        <StaggerContainer className='flex flex-col'>
                            {forcepsIndications.map((item) => (
                    ...
                    <div>
                        <RevealText as="h2" text="How is forceps delivery performed?" className='font-serif text-2xl md:text-3xl text-plum-ink mb-8' />
                        <StaggerContainer className='flex flex-col'>
                            {forcepsSteps.map((step, i) => (
                    ...
                    <div>
                        <RevealText as="h2" text="When is it not advisable?" className='font-serif text-2xl md:text-3xl text-plum-ink mb-6' />
                        <StaggerContainer className='grid sm:grid-cols-2 gap-4'>
                            {forcepsNotAdvisable.map((item) => (
                    ...
                    <div>
                        <RevealText as="h2" text="Benefits of forceps delivery" className='font-serif text-2xl md:text-3xl text-plum-ink mb-6' />
                        <StaggerContainer className='flex flex-col'>
                            {forcepsBenefits.map((item) => (
                                <StaggerItem key={item} className='flex items-start gap-3 py-2.5 border-b border-rose-100 last:border-0'>
                    ...
                    <div>
                        <RevealText as="h2" text="Risks of forceps delivery" className='font-serif text-2xl md:text-3xl text-plum-ink mb-6' />
                        <div className='grid sm:grid-cols-2 gap-6'>
                            <div className='p-6 rounded-card bg-rose-50 border border-rose-100'>
                                <p className='font-serif text-lg text-plum-ink mb-3'>Risks to the mother</p>
                                <div className='flex flex-col gap-2'>
                                    {forcepsMotherRisks.map((item) => (
                                        <p key={item} className='text-sm text-warm-slate leading-relaxed'>{item}</p>
                                    ))}
                                </div>
                            </div>
                            <div className='p-6 rounded-card bg-plum-ink'>
                                <p className='font-serif text-lg text-petal-white mb-3'>Risks to the baby</p>
                                <div className='flex flex-col gap-2'>
                                    {forcepsBabyRisks.map((item) => (
                                        <p key={item} className='text-sm text-rose-100/70 leading-relaxed'>{item}</p>
                                    ))}
                                </div>
                            </div>
                        </div>
                    ...
                    <Reveal>
                        <div className='p-6 md:p-8 rounded-card bg-rose-50 border border-rose-100'>
                            <p className='font-serif text-lg text-plum-ink mb-2'>After the procedure</p>
                            <p className='text-sm text-warm-slate leading-relaxed'>{forcepsAfterProcedure}</p>
                        </div>
                    </Reveal>

                    <div>
                        <RevealText as="h2" text="Postpartum recovery tips" className='font-serif text-2xl md:text-3xl text-plum-ink mb-6' />
                        <StaggerContainer className='flex flex-col'>
                            {forcepsRecoveryTips.map((item) => (
                                <StaggerItem key={item} className='flex items-start gap-3 py-2.5 border-b border-rose-100 last:border-0'>
                                    <span className='mt-0.5 shrink-0 size-5 rounded-full bg-sage-50 text-sage-600 flex items-center justify-center'>
                                        <Check size={12} strokeWidth={3} />
                                    </span>
